...
class LoadProject(Windows):
    ...
    def execute(self, data):
        ...
        b3 = Button(self.root,text="Новый проект1", command=super().nextButton,
               width=15, height=3).place(anchor=CENTER, relx=0.3, rely=0.3)

        b4 = Button(self.root, text="Загрузить проект1", command=super().backButton,
               width=15, height=3).place(anchor=CENTER, relx=0.7, rely=0.3)
...
#TODO убрать поле
    def execute(self, data):
        ...
        self.root.geometry('{}x{}+{}+{}'.format(self.w, self.h, 0, 0))
        ###########################
        Label(self.root, text="1 Шаг",font="Arial 12").grid(row=0, column=0,sticky='w')
        Label(self.root, text="Имя проекта",font="Arial 12").grid(row=1, column=0, padx=2, pady=2, sticky='w')
        ...
        Label(self.root, text="Форма с входными данными, [xlsx]",
                   font="Arial 12").grid(row=3, column=0, padx=2, pady=2, sticky='w')
        ...
        Label(self.root, text="Строительная подоснова, [dwg]",
              font="Arial 12").grid(row=5, column=0, padx=2, pady=2, sticky='w')
        ...
        Label(self.root, text="Чертеж покрасочной кабины, [dwg]",
              font="Arial 12").grid(row=7, column=0, padx=2, pady=2, sticky='w')
        ...
        Button(self.root, text="Открыть", command=lambda: self.openFileButton(".dwg", self.entryPaintDwg)).grid(row=8, column=2, padx=0, pady=2, sticky='w')
        ##########################
...
class GenerateSolution(Windows):
    ...
    def __init__(self):
        ...
        self.update_icon = PhotoImage(file="Files/Update logo.png")
    ...
    def execute(self, data):
        ...
        b7 = Button(self.root, command=self.autoCadCallback,
                    width=100, image=self.autoCad_logo)
        b7.pack(side=RIGHT)

        b8 = Button(self.root, text="Siemens NX", command=self.nxCallback,
                    width=277, image=self.nx_logo)
        b8.pack(side=RIGHT)
        b6 = Button(self.root, command=self.update, width=100, image=self.update_icon)
        b6.pack(side=RIGHT)
